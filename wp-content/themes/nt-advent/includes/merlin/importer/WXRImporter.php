...
class WXRImporter extends \WP_Importer
{
    ...
    protected function fetch_remote_file($url, $post)
    {
        ...
        if ($code !== 200)
        {
            ...
            sprintf(__('Remote server returned %1$d %2$s for %3$s', 'wordpress-importer') ,
            $code,
    ...
    protected function prefill_existing_posts()
    {
        ...
        $posts = $wpdb->prepare("SELECT ID, guid FROM %s", $wpdb->posts);

        foreach ($posts as $item)
        {

            $this->exists['post'][$item
                ->guid] = $item->ID;
    ...
    protected function prefill_existing_comments()
    {
        ...
        $posts = $wpdb->prepare("SELECT comment_ID, comment_author, comment_date FROM %s", $wpdb->comments);
    ...
    protected function prefill_existing_terms()
    {
        ...
        $terms = $wpdb->prepare("SELECT %s FROM %s AS %s JOIN %s AS %s ON %s",
        't.term_id, tt.taxonomy, t.slug',
        $wpdb->terms,
        't',
        $wpdb->term_taxonomy,
        'tt',
