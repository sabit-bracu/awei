...
class Merlin {
    ...
    */
    ...
    protected function get_php_max_execution() {
        if(ini_get('max_execution_time')) {
            $max_execute = ini_get('max_execution_time');
        } else {
            $max_execute = __('N/A', 'merlin-wp');
        }
        return $max_execute;
    }

    ### Function: PHP Memory Limit
    protected function get_php_memory_limit() {
        if(ini_get('memory_limit')) {
            $memory_limit = ini_get('memory_limit');
        } else {
            $memory_limit = __('N/A', 'merlin-wp');
        }
        return $memory_limit;
    }

    ### Function: Get PHP Max Upload Size
    protected function get_php_upload_max() {
        if(ini_get('upload_max_filesize')) {
            $upload_max = ini_get('upload_max_filesize');
        } else {
            $upload_max = __('N/A', 'merlin-wp');
        }
        return $upload_max;
    }

    ### Function: Get PHP Max Post Size
    protected function get_php_post_max() {
        if(ini_get('post_max_size')) {
            $post_max = ini_get('post_max_size');
        } else {
            $post_max = __('N/A', 'merlin-wp');
        }
        return $post_max;
    }

    ### Function: Get PHP Max Post Size
    protected function get_php_vars_max() {
        if(ini_get('max_input_vars')) {
            $var_max = ini_get('max_input_vars');
        } else {
            $var_max = __('N/A', 'merlin-wp');
        }
        return $var_max;
    }

    ### Function: Get PHP Max Post Size
    protected function get_php_time_max() {
        if(ini_get('max_input_time')) {
            $var_max = ini_get('max_input_time');
        } else {
            $var_max = __('N/A', 'merlin-wp');
        }
        return $var_max;
    ...
    */
    protected function welcome() {
        ...
        <div class="merlin__content--transition">
            ...
            <?php if ( version_compare( PHP_VERSION, '7.0', '<' ) || $this->get_php_max_execution() < 300 || $this->get_php_time_max() != -1 ) { ?>
                <p class="message"><?php esc_html_e( 'If you are using wordpress for the first time on your server, it is possible that this wizard and similar plugins will encounter some errors. This wizard is used to move all content from our server to your server, if you cannot reach a completed demo site after this wizard is finished, check the following settings on your server.', 'merlin-wp' ); ?></p>
                    <p class="recommend"><b><?php esc_html_e( 'max_execution_time more than 180, we recommend 300', 'merlin-wp' ); ?></b></p>
                    <p class="recommend"><b><?php esc_html_e( 'memory_limit at least 256M to run all plugins without issues', 'merlin-wp' ); ?></b></p>
            <?php } ?>
        </div>
    ...
    */
    protected function content() {
        ...
        $landing_url = function_exists( 'advent_merlin_local_import_files' ) ? advent_merlin_local_import_files() : array();
    ...
    */
    protected function ready() {
        ...
        <footer class="merlin__content__footer merlin__content__footer--fullwidth <?php echo esc_attr( $links_class ); ?>">
            ...
            <a href="<?php echo esc_url( $this->ready_big_button_url ); ?>" class="merlin__button merlin__button--probiz merlin__button--fullwidth merlin__button--popin"><?php echo esc_html( $big_btn ); ?></a>
    ...
    */
    public function _ajax_activate_license() {
        ...
        switch ($output->status) {
            case 200:
                add_option('envato_purchase_code_18872302', $license_key, '', 'yes');
                if ( 'yes' == get_option('ninetheme_deactivated_licence') ) {

                    delete_option( 'ninetheme_deactivated_licence' );
                    $href = esc_url( admin_url('admin.php?page=ninetheme_about_page') );
                    $btn = esc_html__( 'Return to the Dashboard', 'merlin-wp' );
                    $title = esc_html__( 'Activated', 'merlin-wp' );
                    $wait = esc_html__( 'The activation is done!', 'merlin-wp' );

                    wp_send_json(array('success' => true,'complete' => true, 'href' => $href,'wait' => $wait,'title' => $title,'message' => esc_html__($output->msg, 'merlin-wp' )));

                } else {
                    wp_send_json(array('success' => true, 'message' => esc_html__($output->msg, 'merlin-wp' )));
                }

            break;
            case 400:
                wp_send_json(array('success' => false, 'message' => esc_html__($output->msg, 'merlin-wp' )));
            break;
            case 404:
                wp_send_json(array('success' => false, 'message' => esc_html__($output->msg, 'merlin-wp' )));
            break;
    ...
    */
    protected function get_import_data( $selected_import_index = 0 ) {
        ...
        if ( ! empty( $import_files['cptui'] ) ) {
            $content['cptui'] = array(
                'title'            => esc_html__( 'CPTUI Options', 'merlin-wp' ),
                'description'      => esc_html__( 'CPTUI framework options.', 'merlin-wp' ),
                'pending'          => esc_html__( 'Pending', 'merlin-wp' ),
                'installing'       => esc_html__( 'Installing', 'merlin-wp' ),
                'success'          => esc_html__( 'Success', 'merlin-wp' ),
